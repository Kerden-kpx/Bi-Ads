<template>
  <div class="card">
    <div class="card-header">
      <div class="card-title">FB ADS IMPRESSION & REACH TREND ANALYZE</div>
      <button 
        class="icon-button" 
        @click="handleAnalyze" 
...
export default {
  name: 'Facebook_Analyze_Card_One',
  components: {
    Loading,
    DataAnalysis,
    DocumentChecked,
  ...
  setup(props) {
    ...
    const restoreAnalysisFromCache = () => {
      try {
        const cacheKey = `facebook_impressions_reach_analysis_${props.accountId || 'default'}`
    ...
    const handleAnalyze = async () => {
      ...
      // 如果数据不完整，不触发分析
      if (!props.data || !props.data.impressions || props.data.impressions === 0) {
        return
      }
      ...
      try {
        ...
        // 调用后端 AI 分析接口
        const response = await facebookDashboardAPI.analyzeImpressionsReach(requestData)
