<template>
  <div class="card">
    <div class="card-header">
      <div class="card-title">Purchases Value & Spend Over Time Analyze</div>
      <button 
        class="icon-button" 
        @click="handleAnalyze" 
...
export default {
  name: 'Facebook_Analyze_Card_Two',
  components: {
    Loading,
    DataAnalysis
  },
  ...
  setup(props) {
    ...
    const restoreAnalysisFromCache = () => {
      try {
        const cacheKey = `facebook_purchases_spend_analysis_${props.accountId || 'default'}`
    ...
    const handleAnalyze = async () => {
      ...
      try {
        ...
        // 调用后端 AI 分析接口（购买和花费分析）
        const response = await facebookDashboardAPI.analyzePurchasesSpend(requestData)
