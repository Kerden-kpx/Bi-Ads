<template>
  <div class="card">
    <div class="card-header">
      <div class="card-title">Top Funnel Overview Analyze</div>
      <button 
        class="icon-button" 
        @click="handleAnalyze" 
...
export default {
  ...
  setup(props) {
    ...
    const restoreAnalysisFromCache = () => {
      try {
        const cacheKey = `google_top_funnel_analysis`
    ...
    const handleAnalyze = async () => {
      ...
      try {
        ...
        console.log('Google Top Funnel 分析请求数据:', requestData)
        console.log('传递的指标数据:', props.data)
        
        // 调用后端 AI 分析接口
        const response = await googleDashboardAPI.analyzeTopFunnel(requestData)
